...
class BackyardFlyer(Drone):
    ...
    def is_waypoint_reached(self):
        dist = np.diff(np.subtract(self.local_position[:2], self.all_waypoints[self.waypoint_id]))
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.TAKEOFF:
            ...
            if self.is_waypoint_reached():
                print("waypoint {} is reached".format(self.waypoint_id))
    ...
    def waypoint_transition(self):
        ...
        self.waypoint_id += 1
        if self.waypoint_id == self.all_waypoints.shape[0]:
            self.waypoint_id = 0

        self.target_position[:2] = self.all_waypoints[self.waypoint_id]

        self.cmd_position(self.target_position[0], self.target_position[1], self.target_position[2], 0)

        print("next waypoint ", self.waypoint_id, self.target_position)
